...
function Sidebar() {
  ...
  return (
    <div className={`sidebar ${isClosed ? 'closed' : ''} ${isMobileOpen ? 'open' : ''}`} id="mySidebar">
      ...
      {screenType.includes("UploadedPatient") && (
      <a  className='pe-auto' onClick={Report} title='Upload'><FontAwesomeIcon icon={faUpload} className='me-3 pe-auto' /> <span>Upload</span></a>
      )}
