...
// 2. Fetch data using SIDNo → sets Plan, Gender, Checkup Date
useEffect(() => {
    const fetchSndnoData = async () => {
        if (!SIDNo) return; // Guard clause to avoid unnecessary call

        try {
            const response = await fetch(`${config.apiBaseUrl}/GetSndnoData`, {
                method: "POST",
                headers: {
                    "Content-Type": "application/json",
                },
                body: JSON.stringify({ SID_no: SIDNo }),
            });

            const data = await response.json();
            console.log('SND Data:', data);

            if (Array.isArray(data) && data.length > 0) {
                const [{ plans, gender, checkup_date }] = data;

                setPlan(plans);
                setGender(gender);
                setDate(checkup_date);
            }

        } catch (error) {
            console.error('Error fetching SNDNo data:', error);
        }
    };
...
// 3. Fetch feedback data after Plan and Gender are set
useEffect(() => {
    const fetchFeedbackData = async () => {
        if (!Plan || !gender) return; // Prevent call if values aren't set
